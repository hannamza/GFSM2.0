...
CXListCtrl::CXListCtrl()
{
	...
	m_crBtnFace             = RGB(200,203,240);//::GetSysColor(COLOR_BTNFACE);
	m_crBtnShadow           = ::GetSysColor(COLOR_BTNSHADOW);
	m_crBtnText             = ::GetSysColor(COLOR_BTNTEXT);
	m_crGrayText            = ::GetSysColor(COLOR_GRAYTEXT);
	m_crHighLight           = RGB(200,203,240);//::GetSysColor(COLOR_HIGHLIGHT);//RGB(220,220,250);//
	m_crHighLightText       = RGB(30,30,30);//::GetSysColor(COLOR_HIGHLIGHTTEXT);//RGB(0,0,0);//
		...
		if( nSubItem != -1 )
		{
			...
			if( pXLCD[nSubItem].nCheckedState >= 0 )
			{
				...
				if( pWnd && ::IsWindow(pWnd->m_hWnd) )
				{
					...
					NMI.sText = _T("");
					//nResult = pWnd->SendMessage(WM_XLISTCTRL_CHECKBOX_CLICKING, (WPARAM)&NMI, (LPARAM)nChecked);
					pWnd->PostMessageW(CHECKBOX_CHANGE_NOTIFY, (WPARAM)&NMI, (LPARAM)nChecked);
				}
...
LRESULT CXListCtrl::OnEditChange(WPARAM wParam, LPARAM lParam)
{
	...
	if (pEditBox->GetStyle() & ES_PASSWORD)
	{
		/*pUserInfo pInfo = (pUserInfo) GetItemData(m_nEditItem);
		SAFE_DELETE(pInfo);
		pInfo = new userInfo;
		_tcscpy_s(pInfo->szPW, 64, sText.GetBuffer(0));
		pInfo->bCheck = FALSE;

		SetItemData(m_nEditItem, (DWORD)pInfo);*/

		INT nCnt = sText.GetLength();
		sText = _T("");
		for (int i = 0; i < nCnt; i++)
		{
			sText += _T("*");
	...
	if( pWnd && ::IsWindow(pWnd->m_hWnd) )
		pWnd->PostMessageW(EDITBOX_CHANGE_NOTIFY, (WPARAM)m_nEditItem, (LPARAM)m_nEditSubItem);
...
LRESULT CXListCtrl::OnComboChange(WPARAM wParam, LPARAM lParam)
{
	...
	if (pWnd && ::IsWindow(pWnd->m_hWnd))
		pWnd->PostMessageW(COMBOBOX_CHANGE_NOTIFY, m_nComboItem, m_nComboSubItem);
...
void CXListCtrl::InitList(CXListCtrl& list, const TCHAR** pszTitle)
{
	LV_COLUMN lvcolumn;
	...
	for (int i = 0; ; i++)
	{
		...
		lvcolumn.fmt = LVCFMT_CENTER;// LVCFMT_LEFT;
	...
	// 리스트 폰트 설정
	list.SetTextFont(Global.GetFont(IDX_FONT_SMALL));
	// 언더라인 컬러 설정
	...
	// 윈도우 11에서 헤더 텍스트 삐져 나오는 현상없애기 위한 폰트 적용
