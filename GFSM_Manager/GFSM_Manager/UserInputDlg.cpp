...
IMPLEMENT_DYNAMIC(CUserInputDlg, CSkinDialog)

CUserInputDlg::CUserInputDlg(CWnd* pParent /*=NULL*/)
	: CSkinDialog(IDD_DIALOG_USERINPUT, pParent)
{
...
void CUserInputDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_NAME, m_editWorksite);
	DDX_Control(pDX, IDC_EDIT_PHONE, m_editLoc);
}


BEGIN_MESSAGE_MAP(CUserInputDlg, CSkinDialog)
	ON_WM_DESTROY()
	ON_WM_CTLCOLOR()
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_BUTTON_CONFIRM, &CUserInputDlg::OnBnClickedButtonConfirm)
	ON_BN_CLICKED(IDC_BUTTON_CANCEL, &CUserInputDlg::OnBnClickedButtonCancel)
END_MESSAGE_MAP()


// CUserInputDlg 메시지 처리기입니다.
...
BOOL CUserInputDlg::OnInitDialog()
{
	...
	SetDlgItemText(IDC_EDIT_NAME, m_sName);
	SetDlgItemText(IDC_EDIT_PHONE, m_sPhone);
...
BOOL CUserInputDlg::OnEraseBkgnd(CDC* pDC)
{
...
void CUserInputDlg::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
...
void CUserInputDlg::GetInputValue(CString & sName, CString & sPhone, CString & sEtc)
{
	sName = m_sName;
	sPhone = m_sPhone;
	sEtc = m_sEtc;
...
void CUserInputDlg::SetInputValue(CString sName, CString sPhone, CString sEtc)
{
	m_sName = sName;
	m_sPhone = sPhone;
	m_sEtc = sEtc;
...
void CUserInputDlg::OnBnClickedButtonConfirm()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	GetDlgItemText(IDC_EDIT_NAME, m_sName);
	GetDlgItemText(IDC_EDIT_PHONE, m_sPhone);
...
void CUserInputDlg::OnBnClickedButtonCancel()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	OnCancel();
}
