...
namespace
{
	...
	void PrintConnectionInfo(SOCKET socket)
	{
		...
		trace0("Connection from ip[%s], port[%d] to ip[%s], port[%d] succeeded.", clientIP.c_str(), clientPort, serverIP.c_str(), serverPort);
	}
}
														...
														PVOID Overlapped, ULONG IoResult, ULONG_PTR NumberOfBytesTransferred, PTP_IO /* Io */)
{
	...
	if(IoResult == ERROR_SUCCESS)
	{
		...
		case IOEvent::RECV:
			TRACE("IOEvent::RECV 0\n");
			if(NumberOfBytesTransferred > 0)
			{
				TRACE("IOEvent::RECV 1\n");
				event->client->OnRecv(NumberOfBytesTransferred);
			}
			else
			{
				TRACE("IOEvent::RECV 2\n");
