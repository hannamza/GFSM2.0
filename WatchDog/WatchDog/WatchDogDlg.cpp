...
BOOL CWatchDogDlg::OnInitDialog()
{
	...
	SetDlgItemText(IDC_EDIT_EXE, _T("GFSM_MCS.exe"));
...
void CWatchDogDlg::OnBnClickedButtonStart()
{
	...
	CClientInterface::Instance()->TryConnection("127.0.0.1", m_nServerPort);
...
void CWatchDogDlg::ServerCheckLoop()
{
	...
	if (!m_bFirst) {
		CTime time = CTime::GetCurrentTime();
		CString sTime;
		sTime.Format(L"[EXE]%04d/%02d/%02d %02d:%02d:%02d", time.GetYear(), time.GetMonth(), time.GetDay(), time.GetHour(), time.GetMinute(), time.GetSecond());
	...
	while (g_bThread)
	{
		...
		// 4. Reconnect socket, if not response until 2.5 sec, one time reconnect socket
		if (!bTryReconnect && dwRecvTime >= 2500)
		{
			CClientInterface::Instance()->TryShutdown();
			CClientInterface::Instance()->Destroy();
			CClientInterface::Delete();
		...
		else if(bTryReconnect && dwRecvTime < 2500) {
			bTryReconnect = false;
		}
		// 5. check response
