...
void KISA_SEED_Encrypt_Block_forCBC( DWORD *in, DWORD *out, KISA_SEED_KEY *ks )
{
...
// Set up input values for first round
	L0 = (((BYTE*)in)[3] << 24) | (((BYTE*)in)[2] << 16) | (((BYTE*)in)[1] << 8) | (((BYTE*)in)[0]);
	L1 = (((BYTE*)in)[7] << 24) | (((BYTE*)in)[6] << 16) | (((BYTE*)in)[5] << 8) | (((BYTE*)in)[4]);
	R0 = (((BYTE*)in)[11] << 24) | (((BYTE*)in)[10] << 16) | (((BYTE*)in)[9] << 8) | (((BYTE*)in)[8]);
	R1 = (((BYTE*)in)[15] << 24) | (((BYTE*)in)[14] << 16) | (((BYTE*)in)[13] << 8) | (((BYTE*)in)[12]);
...
void KISA_SEED_Decrypt_Block_forCBC( DWORD *in, DWORD *out, KISA_SEED_KEY *ks )
{
	DWORD L0, L1, R0, R1;		// Iuput/output values at each rounds
	DWORD T0, T1;				// Temporary variables for round function F
	DWORD *K = ks->key_data;		// Pointer of round keys
...
BYTE* int32tochar_for_SEED_CBC( IN DWORD *in, IN int nLen )
{
	...
	data = (BYTE*)malloc(sizeof(BYTE) * nLen);
...
int SEED_CBC_Encrypt( IN BYTE *pbszUserKey, IN BYTE *pbszIV, IN BYTE *pbszPlainText, IN int nPlainTextLen, OUT BYTE *pbszCipherText )
{
	...
	DWORD *outbuf;
	DWORD *data;
	BYTE *newpbszPlainText;
	BYTE *cdata;
	int outlen = 0;
	int nRetOutLeng = 0;
	int nPaddingLeng = 0;
	int nPlainTextPadding = (BLOCK_SIZE_SEED - (nPlainTextLen % BLOCK_SIZE_SEED));
	newpbszPlainText = (BYTE*)malloc(sizeof(BYTE) * (nPlainTextLen + nPlainTextPadding));
	...
	outbuf = (DWORD*)malloc(sizeof(DWORD) * outlen);
...
int SEED_CBC_Decrypt( IN BYTE *pbszUserKey, IN BYTE *pbszIV, IN BYTE *pbszCipherText, IN int nCipherTextLen, OUT BYTE *result )
{
	...
	newpbszCipherText = (BYTE*)malloc(sizeof(BYTE) * (nCipherTextLen));
	memcpy(newpbszCipherText, pbszCipherText, nCipherTextLen);

	pbszPlainText = (BYTE*)malloc(sizeof(BYTE) * (nCipherTextLen));
...
void main()
{
	...
	DWORD *outbuf;
	BYTE *PlainBytes = (BYTE *)(plainText);
	BYTE *CipherBytes = (BYTE *)(pbszCipherText);
	DWORD *data;
	BYTE *cdata;
	int nRetOutLeng = 0;
	int remainleng = 0;
	...
	SEED_CBC_init( &info, KISA_ENCRYPT, pbszUserKey, pbszIV );
	outbuf = (DWORD*)malloc(sizeof(DWORD) * process_blockLeng);		
