...
void Server::OnRecv(IOEvent* event, DWORD dwNumberOfBytesTransfered)
{
	...
	USHORT command = 6;
	int size = 0;
	memcpy_s(&command, sizeof(USHORT), pPacket, sizeof(USHORT));
	memcpy_s(&size, sizeof(UINT), &pPacket[sizeof(USHORT)], sizeof(UINT));
	// 1. protocol check
...
void Server::Send(Packet* packet, BYTE* pData, int nSize)
{
	assert(packet);
	assert(packet->GetSender());
