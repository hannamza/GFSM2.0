...
void CSkinDialog::DrawFrame()
{
	...
	// 이전 브러시 저장
	OldBrush = BufferDC.SelectObject(&myBrush);
	// Rect 그리기
	BufferDC.Rectangle(0, 0, rc.Width(), rc.Height());
	// 이전 브러시 선택  
	...
	// Rect 그리기
	BufferDC.Rectangle(15, 45, rc.Width() - 15, rc.Height() - 25);
	// 이전 브러시, pen 선택  
	BufferDC.SelectObject(OldBrush);
	...
	//// 브러시(해치(패턴)형) 생성
	//myBrush.CreateHatchBrush(HS_DIAGCROSS, RGB(255, 255, 255));
	//// 이전 브러시 저장
	...
	dc.TransparentBlt(0, 0, rc.Width(), rc.Height(), &BufferDC, 0, 0, rc.Width(), rc.Height(), RGB(0, 0, 255));;
			 ...
			 pMsg->message == WM_NCMOUSELEAVE )		// None Client 영역이 아닌곳으로 마우스가 이동됐을 경우
	{
		if( GetWindowLong(GetSafeHwnd(), GWL_STYLE) & WS_CAPTION )
		{
			...
			case WM_NCLBUTTONUP :
				...
				if(m_bClose && rcExit.PtInRect(pMsg->pt) )
				{
				...
				else if(m_bMaximize && rcMaximize.PtInRect(pMsg->pt) )
				{
				...
				else if(m_bMinimize && rcMinimize.PtInRect(pMsg->pt) )
				{
...
void CSkinDialog::SetTitleBar(BOOL bMini, BOOL bMax, BOOL bClose)
{
	m_bMinimize = bMini;
	m_bMaximize = bMax;
	m_bClose = bClose;
...
void CSkinDialog::OnGetMinMaxInfo(MINMAXINFO FAR* lpMMI) 
{
	...
	lpMMI->ptMinTrackSize.x = 800;								// 윈도우 최소 너비 
	lpMMI->ptMinTrackSize.y = 600;								// 윈도우 최소 높이
