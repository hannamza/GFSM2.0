================================================================================
    MFC 라이브러리 : GFSM_SENDER 프로젝트 개요
===============================================================================

응용 프로그램 마법사에서 이 GFSM_SENDER 응용 프로그램을 만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 프로그램을 작성하기 위한 기본 구조를 제공합니다.

GFSM_SENDER 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.
...
GFSM_SENDER.h
    ...
    여기에는 resource.h를 비롯한 다른 프로젝트별 헤더가 포함되어 있으며 CGFSM_SENDERApp 응용 프로그램 클래스가 선언되어 있습니다.

GFSM_SENDER.cpp
    이는 응용 프로그램 클래스 CGFSM_SENDERApp가 포함된 기본 응용 프로그램 소스 파일입니다.
...
res\GFSM_SENDER.ico
    아이콘 파일이며, 응용 프로그램의 아이콘으로 사용됩니다. 이 아이콘은 기본 리소스 파일인 GFSM_SENDER.rc에 의해 포함됩니다.
...
GFSM_SENDERDlg.h, GFSM_SENDERDlg.cpp - 대화 상자
    이 파일에는 CGFSM_SENDERDlg 클래스가 포함됩니다. 이 클래스는 응용 프로그램의 주 대화 상자에 대한 동작을 정의합니다. 이 대화 상자 템플릿은 GFSM_SENDER.rc에 있으며, Microsoft Visual C++에서 직접 편집할 수 있습니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일 GFSM_SENDER.pch와 미리 컴파일된 형식(PCT) 파일 StdAfx.obj를 빌드하는 데 사용됩니다.

Resource.h
    새 리소스 ID를 정의하는 표준 헤더 파일입니다. Microsoft Visual C++에서 이 파일을 읽고 업데이트합니다.

GFSM_SENDER.manifest
